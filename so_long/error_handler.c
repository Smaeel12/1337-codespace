...
/*   Updated: 2025/02/13 10:59:32 by iboubkri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_error(t_error error)
{
	...
	if (error == NOT_RECT_MAP)
		ft_printf("Invalid map format: Map must be rectangular (all rows same length).\n");
	...
	if (error == MAP_INVALID)
		ft_printf("No valid path exists between player, collectibles, and exit.\n");
	if (error == MLX_INS)
